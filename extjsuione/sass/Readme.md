...
folder are mapped to the application's root namespace, 'ExtJSTrainingOne'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ExtJSTrainingOne"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ExtJSTrainingOne' in the same way
as `"ExtJSTrainingOne/sass/src"`.
